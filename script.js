...
document.querySelector('#save a').onclick = function () {
    ...
    var content = document.querySelector('#textbox').value;
    if (document.querySelector('#le-crlf').checked || document.querySelector('#le-dos').checked) {
        content = content.replace(/\n/g, '\r\n');
        if (content.length > 0 && !content.endsWith('\r\n')) content += '\r\n';
    } else {
        if (content.length > 0 && !content.endsWith('\n')) content += '\n';
    }
    if (document.querySelector('#enc-ascii').checked) {
        content = content.replace(/[^\x00-\x7F]/g, '?');
    }
    this.href = URL.createObjectURL(new Blob([content], { type: 'text/plain' }));
};
